...
public class EnemyIdentifierHudMessagePatch {
	...
	public static void StartPostfix(EnemyIdentifier __instance) {
		...
		if (__instance.enemyType == EnemyType.HideousMass) {
			...
			BoolValue bv = mass.gameObject.AddComponent<BoolValue>();
			bv.description = "shownMessage";
		} else if (__instance.enemyType == EnemyType.Stalker) {
			...
			BoolValue bv = stalker.gameObject.AddComponent<BoolValue>();
			bv.description = "shownMessage";
		}/* else if (__instance.enemyType == EnemyType.Sisyphus) {
			...
			BoolValue bv = sisyphus.gameObject.AddComponent<BoolValue>();
			bv.description = "shownMessage";
		}*/
		else if (__instance.enemyType == EnemyType.Idol) {
			...
			BoolValue bv = idol.gameObject.AddComponent<BoolValue>();
			bv.description = "shownMessage";
		} else if (__instance.enemyType == EnemyType.Ferryman) {
			...
			BoolValue bv = ferryman.gameObject.AddComponent<BoolValue>();
			bv.description = "shownMessage";
		}
	}
	...
	public static void UpdatePrefix(EnemyIdentifier __instance) {
		...
		if (__instance.enemyType == EnemyType.HideousMass && SceneHelper.CurrentScene == "Level 1-3" && __instance.transform.root.gameObject.name == "Boss Arena") {
		...
		} else if (__instance.enemyType == EnemyType.Stalker && SceneHelper.CurrentScene == "Level 4-2" && __instance.transform.root.gameObject.name == "5 - Temple Entrance") {
		...
		} else if (__instance.enemyType == EnemyType.Idol && SceneHelper.CurrentScene == "Level 5-2" && __instance.transform.root.gameObject.name == "3 - Ferryman's Cabin") {
		...
		} else if (__instance.enemyType == EnemyType.Ferryman && SceneHelper.CurrentScene == "Level 5-2" && __instance.transform.root.gameObject.name == "8 - Ship" && __instance.gameObject.GetComponent<FerrymanFake>() == null) {
...
public class ObjectActivationCheckPatch {
	...
	public static void StateChangePostfix(ObjectActivationCheck __instance) {
		...
		if (__instance.readyToActivate && SceneHelper.CurrentScene == "Level 4-2" && __instance.transform.root.gameObject.name == "7 - Boss Arena") {
