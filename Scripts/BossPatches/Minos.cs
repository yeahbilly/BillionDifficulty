...
public class MinosPatch {
	...
	public static void SetSpeedPostfix(MinosBoss __instance) {
		if (__instance.difficulty != 19)
			return;
		float hardModeMult = (!Util.IsHardMode()) ? 1.6f : 2f;
		__instance.anim.speed = hardModeMult * __instance.eid.totalSpeedModifier; // BRUTAL: 1.25f
	}
	...
	public static void SpawnBlackHolePostfix(MinosBoss __instance) {
		if (__instance.difficulty != 19)
			return;
		float hardModeMult = (!Util.IsHardMode()) ? 1.8f : 2.3f;
		__instance.currentBlackHole.speed *= hardModeMult; // Brutal: 1.5f
	}
